...
When I was writing {% for post in site.posts %}{% if post.title contains'Unique Binary Search Trees' %}[[LeetCode] Unique Binary Search Trees](/en{{ post.url }}){% endif %}{% endfor %}, I decided to use `\(\LaTeX\)` to write some formulae.
